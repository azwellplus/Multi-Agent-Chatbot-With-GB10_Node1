...
from utils import process_and_ingest_files_background, delete_ingested_files
...
IMAGE_UPLOAD_DIR = os.path.join("uploads", "chat_images")
    ...
    try:
        ...
        while True:
            ...
            if image_id:
                ...
                logger.info(f"[IMAGE_DEBUG] Retrieved image data for image_id: {image_id}, data length: {len(image_data) if image_data else 0}")
                if image_data:
                    logger.info(f"[IMAGE_DEBUG] image_data preview: {image_data[:100]}...")
                else:
                    logger.warning(f"[IMAGE_DEBUG] No image_data retrieved from postgres for image_id: {image_id}")

            try:
                logger.info(f"[IMAGE_DEBUG] Calling agent.query with image_data: {bool(image_data)}")
                ...
                await websocket.send_json({"type": "error", "data": f"Error processing request: {str(query_error)}"})
...
async def upload_image(request: Request, image: UploadFile = File(...), chat_id: str = Form(...)):
    ...
    original_extension = Path(image.filename or "").suffix
    extension = original_extension or guessed_extension or ".png"

    image_id = str(uuid.uuid4())
    file_name = f"{image_id}{extension}"
    file_path = Path(IMAGE_UPLOAD_DIR) / file_name
    ...
    try:
        ...
        logger.error(f"Failed to write uploaded image to disk: {write_error}")
    ...
    storage_origin = (os.getenv("PUBLIC_BACKEND_ORIGIN") or "").strip().rstrip("/")
    if not storage_origin:
        storage_origin = (os.getenv("STORAGE_BASE_URL") or "").strip().rstrip("/")

    if storage_origin:
        image_url = f"{storage_origin}/chat_images/{file_name}"
    else:
        image_url = str(request.url_for("chat_images", path=file_name))
...
@app.delete("/ingest/{task_id}")
async def delete_ingest_task(task_id: str):
    """Delete uploaded files associated with an ingestion task.
    
    Args:
        task_id: Unique task identifier whose uploads should be deleted
    """
    try:
        log_request({"task_id": task_id}, f"/ingest/{task_id}")
        result = await delete_ingested_files(task_id, config_manager, indexing_tasks, vector_store)
        response = {
            "status": "success",
            "message": f"Deleted uploads for task {task_id}",
            "task_id": task_id,
            "deleted_files": result.get("deleted_files", []),
            "deleted_vector_count": result.get("deleted_vector_count", 0)
        }
        log_response(response, f"/ingest/{task_id}")
        return response
    except FileNotFoundError as e:
        log_error(e, f"/ingest/{task_id}", {"task_id": task_id})
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
        log_error(e, f"/ingest/{task_id}", {"task_id": task_id})
        raise HTTPException(status_code=500, detail=f"Failed to delete uploads for task {task_id}: {str(e)}")
...
async def get_sources():
    ...
    try:
        ...
        uploads_index: Dict[str, str] = {}

        if os.path.isdir(uploads_root):
            for task_folder in os.listdir(uploads_root):
                task_path = os.path.join(uploads_root, task_folder)
                if not os.path.isdir(task_path):
                    continue
                for _, _, files in os.walk(task_path):
                    for file_name in files:
                        uploads_index.setdefault(file_name, task_folder)

        enriched_sources = [
            {
                "name": source_name,
                "task_id": uploads_index.get(source_name),
            }
