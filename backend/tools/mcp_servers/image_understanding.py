...
# Support external vision model server via environment variable
VISION_MODEL_BASE_URL = os.getenv("VISION_MODEL_BASE_URL", "http://qwen2.5-vl:8000/v1")
print(f"[image_understanding] Using vision model at: {VISION_MODEL_BASE_URL}")
...
def explain_image(query: str, image: str):
    ...
    try:
        print(f"[image_understanding] Sending request to vision model: {query}")
        print(f"[image_understanding] Vision model URL: {VISION_MODEL_BASE_URL}")
        ...
        print(f"[image_understanding] Received response from vision model")
        return response.choices[0].message.content
    except Exception as e:
        import traceback
        error_details = traceback.format_exc()
        print(f"[image_understanding] Error calling vision model: {e}")
        print(f"[image_understanding] Error details: {error_details}")
        print(f"[image_understanding] Attempted URL: {VISION_MODEL_BASE_URL}")
